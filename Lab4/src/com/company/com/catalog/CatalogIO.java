...
//		int mod=switchMod?1:0;
        switch (switchMod ? 1 : 0) {
            case (1): {
                switch (catalog.get(name).getPathImage().startsWith("C:") ? 1 : 0) {
                    case 1:
                        file = new File(catalog.get(name).getPathImage());
                        break;
                    case 0:
                        file = new File(path.toString() + "/" + catalog.get(name).getPathImage());
                        break;
                }
            }
            case 0: {
                switch (catalog.get(name).getPathDefinition().startsWith("C:") ? 1 : 0) {
                    case 1:
                        file = new File(catalog.get(name).getPathDefinition());
                        break;
                    case 0:
                        file = new File(path.toString() + "/" + catalog.get(name).getPathImage());
                        break;
    ...
    public void writePdf(String path) {
        WriterPdf writer = new WriterPdf(path);
        writer.run(catalog);
    }
